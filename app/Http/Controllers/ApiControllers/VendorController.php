...
class VendorController extends Controller
{
    ...
    public function addVendorProduct(Request $request)
    {
        try {
            ...
            // /** @var \App\Models\Vendor $vendor */
            $vendor = auth('vendor')->user();
            Log::info('AddVendorProduct: Auth attempt', [
                'vendor_id' => $vendor ? $vendor->id : null,
                'is_active' => $vendor ? $vendor->is_active : null,
                'ip' => $request->ip(),
            ]);
            ...
            if (!$vendor || !$vendor->is_active || !$vendor->is_approved) {
                Log::warning('AddVendorProduct: Authentication failed or vendor inactive/unapproved', [
                    'vendor_id' => $vendor ? $vendor->id : null,
                ]);
                return response()->json([
                    'message' => 'Permission Denied!',
    ...
    public function homeData(Request $request)
    {
        try {
            ...
            // /** @var \App\Models\Vendor $vendor */
            $vendor = auth('vendor')->user();
            Log::info('HomeData: Auth attempt', [
                'vendor_id' => $vendor ? $vendor->id : null,
                'is_active' => $vendor ? $vendor->is_active : null,
                'ip' => $request->ip(),
            ]);
            ...
            if (!$vendor || !$vendor->is_active || !$vendor->is_approved) {
                Log::warning('HomeData: Authentication failed or vendor inactive/unapproved', [
                    'vendor_id' => $vendor ? $vendor->id : null,
                ]);
                return response()->json([
                    'message' => 'Permission Denied!',
    ...
    public function updateProfile(Request $request)
    {
        try {
            ...
            if (!$vendor || !$vendor->is_active || !$vendor->is_approved) {
                Log::warning('UpdateProfile: Authentication failed or vendor inactive/unapproved', [
                    'vendor_id' => $vendor ? $vendor->id : null,
                ]);
                return response()->json([
                    'message' => 'Permission Denied!',
